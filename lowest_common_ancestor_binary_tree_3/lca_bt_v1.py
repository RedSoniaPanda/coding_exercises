...
class Solution:
    def get_ancestors(self, node):
        if node.parent is None:
            ...
            return [node] + self.get_ancestors(node.parent)

    def lowestCommonAncestor(self, p: 'Node', q: 'Node') -> 'Node':
        # A node can be considered a descendent of itself, which means the node passed in could be the answer
        p_ancestors = self.get_ancestors(p)
        q_ancestors = self.get_ancestors(q)

        common_ancestors = []
        for p_anc in p_ancestors:
            for q_anc in q_ancestors:
                if p_anc == q_anc:
                    common_ancestors.append(p_anc)
